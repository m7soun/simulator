...
use App\Services\Requests\V1\Templates\Drivers\Authentication\Logout;
...
class LoginOutAction implements Action
{
    ...
    public function execute($data = null): void
    {
        ...
        if (($now < $shiftStart || $now > $shiftEnd) && is_null($order)) {
            Client::create(new Logout($this->getEntity()))->call();
            $this->getEntity()->setStatus(Driver::STATUS_NOT_AVAILABLE);
            error_log("PID - " . getmypid() . " - " . $this->getEntity()->getDriverId() . " log out");
            sleep(1800);
            exit(0);
        }
    }
